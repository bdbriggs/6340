...
error DFCNFTContract__MaxSupplyReached();
error DFCNFTContract__ValueNotEqualPrice();
error DFCNFTContract__WrongAvenueForThisTransaction();

/// @custom:security-contact dee.woah@gmail.com
contract DFCNFTContract is 
    ERC721, 
    ERC721Enumerable, 
    ERC721URIStorage, 
    ERC721Burnable,
    ...
    ReentrancyGuard 
{
    ...
    constructor(        
        ...
        ) ERC721("DFCNFTContract", "DFC") Ownable(msg.sender) {
    ...
    receive() external payable {
        revert DFCNFTContract__WrongAvenueForThisTransaction();
    }
   ...
   function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert DFCNFTContract__MaxSupplyReached();
        }
        // make sure there us the right amount of money
        if (msg.value != i_mint_price) {
            revert DFCNFTContract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
